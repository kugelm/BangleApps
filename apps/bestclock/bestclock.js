...
const settings = require("Storage").readJSON("bestclock.json", 1)||{};
...
function drawBlink(onoff) {
  const BLACK = 0,
        BLINK = 0x600;
  if (onoff) {
      g.setColor(BLINK).fillCircle(16, 210, 8);
  } else {
      g.setColor(BLACK).fillRect(8,202,24,218);
  }
}
